...
- 🔑 **마스터 비밀번호 로그인**: 앱 실행 후 인증된 사용자만 사용 가능
- ⏸️ **일시정지/재개**: 중단된 작업 이어서 진행 가능
- 📝 **상세 로깅**: 모든 작업 이력 기록 및 조회

## 보안 / 로그인

- 최초 실행 시 마스터 비밀번호를 1회 설정합니다.
- 이후에는 로그인해야 메인 화면에 진입할 수 있습니다.
- 저장된 연결 프로필/프리셋은 마스터 비밀번호로부터 파생한 키로 보호됩니다.
- 기존 프로필/프리셋이 있으면 최초 설정 시 새 키로 자동 재암호화됩니다.
...
1. 처음 실행하면 마스터 비밀번호를 설정합니다.
2. 이후 실행부터는 마스터 비밀번호를 입력해야 앱을 사용할 수 있습니다.
...
1. 생성한 프로필 선택 후 "마이그레이션 작업 설정" 클릭
2. 달력에서 날짜 범위 선택
3. 파티션 목록 확인
4. "시작" 버튼 클릭
