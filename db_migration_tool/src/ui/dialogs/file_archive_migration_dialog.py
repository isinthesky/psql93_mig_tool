...
class FileArchiveMigrationDialog(ScanHostMixin, QDialog):
    ...
    def __init__(self, parent, profile: ConnectionProfile, restricted: bool = False):
    ...
    def _set_run_state(self, state: str, detail: str = ""):
        ...
        self.start_btn.setEnabled(
            state in ("idle", "stopped", "partial", "failed") and not license_blocked
        )
        self.start_btn.setToolTip(
            "라이선스 제한 모드입니다. 중단된 작업의 재개만 시작할 수 있습니다."
            if license_blocked
            else "선택한 범위와 파티션으로 마이그레이션을 시작합니다."
        )
    ...
    def start_migration(self):
        ...
        if self.history_id and not self.resume_mode:
            ...
            if check.pending:
                self.resume_mode = True
                self._frozen_selection = list(check.pending)
                self.add_log(f"중단된 작업 재개: 미완료 파티션 {len(check.pending)}개", "INFO")
        ...
        # legacy 채택 때 보충한 '있을 수 있는 이름'만 원본에 없으면 0건 완료(H-09 리뷰).
        # 원래 checkpoint·새 계획의 파티션이 없으면 여전히 실패한다.
        self.worker.absent_ok_partitions = self._legacy_supplemented(self.history_id)
    ...
    def _legacy_supplemented(self, history_id: int) -> frozenset[str]:
        """이전 버전 작업을 채택할 때 보충한 파티션 이름. 모르면 빈 집합(엄격)."""
        try:
            history = self.history_manager.get_history(history_id)
        except Exception:
            return frozenset()
        if history is None or history.legacy_adopted_at is None:
            return frozenset()
        return frozenset(history.legacy_supplemented)
