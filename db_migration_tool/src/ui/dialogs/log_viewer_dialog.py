...
from PySide6.QtWidgets import (
    ...
    QLabel, QDateEdit, QToolBar, QSplitter
)
from PySide6.QtCore import Qt, QTimer, QDate, Signal, Slot
from PySide6.QtGui import QTextCursor, QAction, QIcon, QTextCharFormat, QColor
from datetime import datetime, timedelta

from src.database.local_db import get_db, LogEntry
...
class LogViewerDialog(QDialog):
    ...
    def update_session_list(self):
        ...
        try:
            ...
            sessions = session.query(
                LogEntry.session_id,
                session.query(LogEntry.timestamp).filter(
                    LogEntry.session_id == LogEntry.session_id
                ).order_by(LogEntry.timestamp).limit(1).scalar_subquery().label('start_time')
            ).filter(
                LogEntry.timestamp >= recent_date,
                LogEntry.session_id.isnot(None)
            ).group_by(LogEntry.session_id).order_by('start_time').all()
    ...
    def append_log_entry(self, log_entry):
        ...
        # HTML 형식으로 추가
        html = f'<span style="color: {color}">[{timestamp}] [{session_id}] [{level}] {message}</span>'
