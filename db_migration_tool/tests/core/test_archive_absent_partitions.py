...
규칙
- 원본 부재를 0건 완료로 보는 것은 **legacy 채택 때 보충한 이름**(`absent_ok_partitions`)만이다.
  같은 이력이라도 원래 checkpoint는 구버전이 실제 탐색·manifest에서 고른 실존 파티션이므로,
  없어졌다면 잘못된 원본·아카이브를 가리킨다는 신호다 — 여전히 실패한다(리뷰 라운드 1).
  새로 만든 계획도 원본 목록에서 고른 파티션이므로 같다.
...
# legacy 이력에 원래 있던 checkpoint(채택 때 보충한 이름이 아님).
ORIGINAL = "point_history_240103"
...
class DummyConn:
    def __init__(self, exists: dict[str, bool] | None = None, fail: Exception | None = None):
        ...
        self.rollbacks = 0
        # 앞 파티션의 실패로 트랜잭션이 중단된 상태(PostgreSQL: rollback 전까지 모든 명령 거부).
        self.aborted = False
...
def _import_worker(tmp_path, names: list[str], *, absent_ok: frozenset[str] = frozenset()):
    ...
    worker.absent_ok_partitions = frozenset(absent_ok)
...
class TestImportAbsentPartition:
    def test_adopted_legacy_absent_partition_completes_with_zero_rows(self, tmp_path):
        worker = _import_worker(tmp_path, [PRESENT, ABSENT], absent_ok=frozenset({ABSENT}))
    ...
    def test_absent_partition_without_legacy_adoption_still_fails(self, tmp_path):
        worker = _import_worker(tmp_path, [ABSENT])
    ...
    def test_manifest_gap_with_file_on_disk_is_not_absence(self, tmp_path):
        ...
        worker = _import_worker(tmp_path, [ABSENT], absent_ok=frozenset({ABSENT}))
        worker.archive_store.load = Mock(return_value=_manifest())
    ...
    def test_listed_partition_with_missing_file_still_fails(self, tmp_path):
        ...
        worker = _import_worker(tmp_path, [PRESENT], absent_ok=frozenset({PRESENT}))
        worker.archive_store.load = Mock(return_value=_manifest(PRESENT))
    ...
    def test_original_checkpoint_missing_from_archive_still_fails(self, tmp_path):
        """리뷰 라운드 1: 채택한 이력이라도 **원래 checkpoint**가 아카이브에 없으면 실패한다.

        원래 checkpoint는 구버전이 manifest 목록에서 고른 실존 파티션이다. 없다면 다른 기간의
        아카이브를 골랐다는 신호다(legacy는 identity를 확인할 수 없어 마지막 방어선).
        """
        worker = _import_worker(tmp_path, [ORIGINAL, ABSENT], absent_ok=frozenset({ABSENT}))
        worker.archive_store.load = Mock(return_value=_manifest())

        with pytest.raises(ValueError, match=f"{ORIGINAL}: manifest에 없음") as exc:
            worker._execute_migration()

        assert ABSENT not in str(exc.value)
        worker._import_partition.assert_not_called()
        assert not _completed_zero(worker, ORIGINAL)
        assert not _completed_zero(worker, ABSENT)

    def test_supplement_log_names_only_supplemented_partitions(self, tmp_path):
        worker = _import_worker(tmp_path, [PRESENT, ABSENT], absent_ok=frozenset({ABSENT}))
        worker.archive_store.load = Mock(return_value=_manifest(PRESENT))
        worker.archive_store.verify_partition_file = Mock()
        logs: list[str] = []
        worker.log.connect(lambda message, _level: logs.append(message))

        worker._execute_migration()

        supplemented = [m for m in logs if "보충한" in m]
        assert supplemented and all(ABSENT in m for m in supplemented)
        assert not any(PRESENT in m for m in supplemented)
...
def _export_worker(
    tmp_path, names: list[str], conn: DummyConn, *, absent_ok: frozenset[str] = frozenset()
):
    profile = ConnectionProfile(
        id=1,
        name="t",
...
class TestExportAbsentPartition:
    def test_adopted_legacy_absent_source_table_completes_with_zero_rows(self, tmp_path):
        ...
        worker = _export_worker(tmp_path, [PRESENT, ABSENT], conn, absent_ok=frozenset({ABSENT}))
        ...
        # 존재 확인은 보충한 이름에만 한다(원래 파티션은 확인 없이 export — 없으면 실패).
        assert [params for _sql, params in conn.queries] == [("public", ABSENT)]
    ...
    def test_existence_check_failure_is_not_absence(self, tmp_path):
        ...
        worker = _export_worker(tmp_path, [ABSENT], conn, absent_ok=frozenset({ABSENT}))
    ...
    def test_without_legacy_adoption_absence_is_not_checked(self, tmp_path):
        ...
        worker = _export_worker(tmp_path, [ABSENT], conn)
    ...
    def test_original_checkpoint_missing_from_source_is_not_completed(self, tmp_path):
        """리뷰 라운드 1: 원래 checkpoint는 존재 확인으로 건너뛰지 않는다 — export에서 실패한다."""
        conn = DummyConn(exists={ORIGINAL: False, ABSENT: False})
        worker = _export_worker(tmp_path, [ORIGINAL, ABSENT], conn, absent_ok=frozenset({ABSENT}))
        worker._export_partition = Mock(
            side_effect=psycopg2.ProgrammingError(f'relation "{ORIGINAL}" does not exist')
        )

        with pytest.raises(psycopg2.ProgrammingError):
            worker._execute_migration()

        assert [c.args[0] for c in worker._export_partition.call_args_list] == [ORIGINAL]
        assert all(params != ("public", ORIGINAL) for _sql, params in conn.queries)
        assert not _completed_zero(worker, ORIGINAL)

    def test_supplement_after_a_skipped_failure_is_still_checked(self, tmp_path):
        """'오류 시 건너뛰기'에서 앞 파티션 export가 실패하면 원본 트랜잭션이 중단 상태로 남는다.

        다음 보충분의 존재 확인은 그 상태를 먼저 닫아야 한다 — 아니면 'current transaction is
        aborted'로 실패해 없는 파티션도 끝나지 않는다(실DB 확인에서 발견).
        """
        conn = DummyConn(exists={ORIGINAL: False, ABSENT: False})
        worker = _export_worker(tmp_path, [ORIGINAL, ABSENT], conn, absent_ok=frozenset({ABSENT}))
        worker.skip_on_error = True

        def fail_and_abort(*_args):
            conn.aborted = True
            raise psycopg2.ProgrammingError(f'relation "{ORIGINAL}" does not exist')

        worker._export_partition = Mock(side_effect=fail_and_abort)

        worker._execute_migration()

        assert [f["partition"] for f in worker.partition_failures] == [ORIGINAL]
        assert _completed_zero(worker, ABSENT)
        assert not _completed_zero(worker, ORIGINAL)
