...
def verify_chunked_copy():
    ...
    with patch("src.core.copy_migration_worker.TableCreator") as MockTableCreator:
        ...
        postgres_utils_mock.PostgresOptimizer.estimate_table_size.return_value = {
            "exists": True,
            "row_count": 5,
            "total_size_mb": 1.0,
        }
        ...
        print(
            f"Checkpoint update count: {worker.checkpoint_manager.update_checkpoint_status.call_count}"
        )

        # Verify the last checkpoint update
        last_call = worker.checkpoint_manager.update_checkpoint_status.call_args_list[
            -2
        ]  # Last one is 'completed', second to last is running update
